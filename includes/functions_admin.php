...
* Lock topic(s)
*/
function lock_topics($where_ids, $ids)
{
If (!$ids)
{
}
...
* Lock function
*/
function lock($forum_id, $prune_mode, $prune_date, $prune_flags = 0, $auto_sync = true, $prune_move = false)
{
	global $db;
...
return lock_topics('topic_id', $topic_list);
	//return delete_topics('topic_id', $topic_list, $auto_sync, false);
}
...
function auto_prune($forum_id, $prune_mode, $prune_flags, $prune_days, $prune_freq)
{
	...
	$sql = 'SELECT forum_name, prune_move
		FROM ' . FORUMS_TABLE . "
		WHERE forum_id = $forum_id";
	...
	if ($row)
	{
		...
		prune($forum_id, $prune_mode, $prune_date, $prune_flags, true, $row['prune_move']);
