...
class acp_lock
{
	...
	function main($id, $mode)
	{
		...
		{
			case 'forums':
				$this->tpl_name = 'acp_lock_forums';
				$this->page_title = 'ACP_LOCK_FORUMS';
			...
			if (confirm_box(true))
			{
				...
				if ($row = $db->sql_fetchrow($result))
				{
					...
					do
					{
						...
						if ($prune_all)
						{
							$p_result = lock($row['forum_id'], 'posted', time(), $prune_flags, false);
						...
						else
						{
							if ($prune_posted)
							{
								$return = lock($row['forum_id'], 'posted', $prunedate_posted, $prune_flags, false);
							...
							if ($prune_viewed)
							{
								$return = lock($row['forum_id'], 'viewed', $prunedate_viewed, $prune_flags, false);
								$p_result['topics'] += $return['topics'];
								$p_result['posts'] += $return['posts'];
							}
						}
					...
					add_log('admin', 'LOG_LOCKS', $log_data);
				}
				$db->sql_freeresult($result);
...
$where_sql .= ($occupation) ? ' AND user_occ ' . $db->sql_like_expression(str_replace('*', $db->any_char, utf8_clean_string($occupation))) : '';
